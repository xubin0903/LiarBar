...
桌上 UI 拍（**不是**新 `phase`）：ConfirmReady → **PlayLaunch** → **PlayLanded** → **AwaitChallenge** 入口，见 [出牌扣牌-状态机](../02-游戏设计/出牌扣牌-状态机.md)。

| 层 | 口径 |
|----|------|
| UI | 点确认 **立刻** `PlayLaunch`（乐观飞）；飞窗 320ms（280～400）不可跳过 |
| T20 | 仍是权威 `SUBMIT_PLAY`。接受才写 `Play` / `lastPlay` / 轮转 |
| 拒 | 回滚 pile+hand，桌上只出「出牌未生效 / 张数已纠正」 |
| 等 ack 才飞 | **禁**（弱网会变成无飞出瞬间提交） |
| AwaitChallenge | PlayLanded 之后的入口。**质疑已解冻**；旧冻轨与新轨 **禁并存**。四拍 / 裁定仍认 T30–T33，演出第2节 |

实现不得把 `PlayLaunch` / `PlayLanded` / `AwaitChallenge` 加成 §1 阶段枚举。
