...
| T12 | `TURN` | `INTENT_CHALLENGE` | 当前座位 | **`!canChallenge`**（第一手或无上家本手） | `TURN` | **忽略**；UI 为 `DISABLED_FIRST_PLAY` / `DISABLED_NO_TARGET` |
...
| # | 从 | 事件 | 触发者 | 条件 | 到 | 副作用摘要 |
|---|----|------|--------|------|----|------------|
| T30 | `CHALLENGE_RITUAL` | `RITUAL_DONE` / `SKIP_RITUAL` | 荷官/玩家 | 总长 ≤ `challenge_ritual_max_seconds`（4s） | `JUDGE` | 拍 2 押注窗：拍 3 开始 **或** 满 `challenge_bet_seconds`（2s）先到先关；押注 **不改扣谁命** |
...
*相关：[01-工程脚手架约定](./01-工程脚手架约定.md) · [GDD](../02-游戏设计/GDD.md) · [数值表](../02-游戏设计/数值与牌堆配置.md)*
