...
**壳层 id（引用 UI 表，不另起）**：阶段仍用上表；页面/封面用 `lb_scr_*`。质疑仪式表现态只用 UI 已定后缀。

| 阶段 / 窗 | 屏或容器 id | 表现态后缀（美术/控件） |
|-----------|-------------|-------------------------|
| `LOBBY` | `lb_scr_lobby`（入口 `lb_scr_home`） | `_idle` |
| `DEAL` `CLAIM` `TURN` `PENALTY` | `lb_scr_table` | 看牌叠 `lb_scr_peek` |
| `TURN` + `CHALLENGE_ONLY` | 仍 `lb_scr_table`；`lb_btn_challenge` | 手牌 `_empty`；文案 `lb_str_hand_empty` |
| `PLAY_REVEAL_SELF` | `lb_scr_play` / `lb_sheet_play` | `_soft` `_slam` `_hesitate` |
| `CHALLENGE_RITUAL` 拍1 | `lb_scr_challenge` / `lb_cover_challenge` | **`_challenge_windup`** |
| `CHALLENGE_RITUAL` 拍2 + 拍3 翻 | 同上 | **`_challenge_reveal`**（对峙不另造 `_standoff`） |
| `CHALLENGE_RITUAL` 定格 / `JUDGE` | 同上 | **`_challenge_result`**；跳过 `_challenge_skip` |
| `JUDGE` 成功/失败 | 角色/顶区 | `_challenge_win` / `_challenge_lose`（只示意，不改裁定） |
| `RECAP` | `lb_scr_recap` | `_win` / `_lose` |
...
| # | 从 | 事件 | 触发者 | 条件 | 到 | 副作用摘要 |
|---|----|------|--------|------|----|------------|
| T30 | `CHALLENGE_RITUAL` | `RITUAL_DONE` / `SKIP_RITUAL` | 荷官/玩家 | 总长 ≤ `challenge_ritual_max_seconds`（4s） | `JUDGE` | 表现态 `_challenge_windup` → `_challenge_reveal` → `_challenge_result`（或 `_challenge_skip`）；拍 2 押注 `lb_cmp_bet_bar`：拍 3 开始 **或** 满 `challenge_bet_seconds`（2s）先到先关；押注 **不改扣谁命** |
...
| T33 | `PENALTY` | `PENALTY_DONE` | 荷官 | `aliveCount == 1` | `RECAP` | 唯一有命者胜；写战报字段（GDD §12） |
| T40 | `RECAP` | `PLAY_AGAIN` | 开桌人 | — | `LOBBY` 或直接 `DEAL`（实现自选，须清空手牌/宣称） | 新 `Match` |
| T41 | `RECAP` | `LEAVE` | 开桌人 | — | `END` | 写本地战绩 |
| T42 | `END` | `TO_HOME` | 玩家 | — | `LOBBY` | — |
...
权威：GDD §10.1。下列四条必须有单测；**禁止**实现「手牌多者胜」。
...
| `winnerSeatId` | SeatId \| null | 仅 `RECAP`/`END` |
...
| 2026-09-06 | 壳层对齐 UI `lb_scr_*` 与质疑三拍后缀 | 不改转移语义 | 客户端文档 | 待会签 |
