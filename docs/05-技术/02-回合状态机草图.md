...
桌上 UI 拍（**不是**新 `phase`）：ConfirmReady → **PlayLaunch** → **PlayLanded** →（hand>0）**AwaitChallenge** /（hand==0）**RankSettle**，见 [出牌扣牌-状态机](../02-游戏设计/出牌扣牌-状态机.md)。
...
| 等 ack 才飞 | **禁** |
| 分流 | PlayLanded 后 hand>0 → AwaitChallenge；**hand==0 → RankSettle，禁止 AwaitChallenge**。打光者不再出牌、该手不再给下游质疑 |
