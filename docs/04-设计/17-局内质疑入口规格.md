...
|----|------|
| 版本 | **v0.3.6** |
| 状态 | **第2节入口会签草案**（PM）· **v0.3.0 废「真/假」同义质疑**：**质疑\|相信** 同层二键 · **质疑 ≠ 相信** · **v0.3.1 Rebuild F2/F4**：ChallengeCommit 全员可见层 · 本局宣称+轮到谁改左上 · 线框闸 · **靶 = 池上本手** · **v0.3.2 R2**：第二人可质疑第一手 / 废第一手禁质疑 · **v0.3.3** 交叉 18 时长 · **v0.3.4 R5**：AwaitChallenge 池上本手牌背可见·张数=`lastPlay.count` · **R6 交叉 11**：法官/荷官句全左上 · 中顶仅倒计时 · **v0.3.5**：F17-28/29/30 交叉 **S18-26～28**（去「待策划补」）· **v0.3.6**：交叉桌槌 **[17a](./17a-局内桌槌表现规格.md) v0.1.0** / 策划 [#236](https://github.com/xubin0903/LiarBar/pull/236) **S19-1～6**（入口语义一字不改）· 生命交叉烛火主读 · 未会签禁止落 ets |
...
| 20 | **桌槌表现（v0.3.6 · 旁挂 17a）** | 质疑=该座锤抬砸；相信=同槌平放搁置。线框数字 / 锚 / 层序 / 调试挂点认 **[17a](./17a-局内桌槌表现规格.md)**。入口语义、身前二键、超时=相信、GAP=24%、禁抬 `padB`、开牌 1000/3000、R6 中顶只 timer **不动** |
...
### 3.9 桌槌表现（v0.3.6 · 线框认 17a · 入口语义不动）

Aron A：质疑=该座木槌抬砸；相信=同槌平放搁置。人模静态贴图 **零改**。全员同轨（F2）。**详细锚点 / 层序 / 调试挂点 = [17a v0.1.0](./17a-局内桌槌表现规格.md)**（对齐策划 [#236](https://github.com/xubin0903/LiarBar/pull/236) / S19-1～6，**勿另造短句 key**）。

| 锁 | 数字 / id |
|----|-----------|
| 层 | `lb_cmp_choice_act`：全员桌面层；`HitTest None`；**池叠之上**；手牌行/确认条可短暂盖字**不挡点** |
| 正式 / 调试 | 正式：Commit / Trust **ACK 后全员播**该行动座锤。调试：立刻播 **本地玩家座位** |
| 质疑=砸 | 锚=行动座身前（自机：环–手带之间、入口二键簇 **上沿外 8～16vp**；侧/对家镜像 seat 锚）。短弧抬→砸；主读定格 **0.8～1.2s** 后收；**Reveal 起势前须收** |
| 相信=搁 | 同槌平放，键簇旁 **外侧分槽**（自机 BottomEnd/键旁；侧家 seat 角）。静止或极轻一顿。与砸 **落点分槽**；禁同姿仅换色 |
| 禁压 | `lb_cmp_pool` / 手牌主行 / 烛 `lb_cmp_life_*` |
...
| 挂点口径 | **无徽只改左上字 = 不过**（并入 S19-1/2）。飞词若做禁挡池（P0 可后置） |
...
**软挂点：** develop 已合 [#239](https://github.com/xubin0903/LiarBar/pull/239)（id 对齐）。定格 / 锚数字以本表 + 17a 为准；代码现 `HOLD_MS=360`、`SELF_SIDE_VP=64`。本票 **零 ets**。
...
| F17-34 | **砸与搁不可辨**（S19-4 `lb_str_choice_act_same_pose`） | 两态同姿仅换色 / 落点不分槽 |
| F17-35 | **锤挡池或手**（S19-5 `lb_str_choice_act_blocks`） | 砸轨或搁位置压 `lb_cmp_pool` / 手牌行 / 烛 |
...
| **v0.3.6** | 2026-09-19 | **桌槌交叉（对齐 #236 / 17a v0.1.0）**：§3.9 锁砸/搁线框挂点（锚 8～16vp · 定格 0.8～1.2s · `lb_cmp_choice_act` · 调试二键）；F17-31～36 = **S19-1～6**（不另造 key）。入口语义 / GAP=24% / 禁抬 padB / 开牌 1000/3000 / R6 **一字不改**。零 ets |
...
| 2026-09-19 | Aron A：锤表现立刻干；UI 写线框/锚/层序/调试挂点；对齐 #236 S19 | v0.3.6 §3.9 + [17a](./17a-局内桌槌表现规格.md) v0.1.0 | UI 规格岗 | **本 PR 写入 · 待会签** |

---

*维护人：UI 文档岗 · 2026-09-19 · docs only · 第2节入口会签草案 **v0.3.6** · 桌槌交叉 **17a v0.1.0** / **S19-1～6** · **质疑\|相信** · **R2** · **R5** 本手可见 · **R6** 法官句左上 · F17-28/29/30=**S18-26～28** · 交叉 18 **v0.1.6** 1000/3000 · 靶=池 · 会签前零 ets · 合入 ≠ 终验*
