...
| 20 | **桌槌表现（v0.3.10 · 旁挂 17a v0.2.2）** | 四座常驻；质疑=就地砸回搁；相信=槌不动。分锚 / 尺 / 层序 / 调试认 **[17a](./17a-局内桌槌表现规格.md) v0.2.2**（左座=头像右空朝桌；右座=头像左空朝桌；对家=左空离脸；自机=贴底座头像旁；常驻 **96～120vp** / client 112）。入口语义、身前二键、超时=相信、GAP=24%、禁抬 `padB`、开牌 1000/3000、R6 中顶只 timer **不动** |
...
### 3.9 桌槌表现（v0.3.10 · 线框认 17a v0.2.2 · 入口语义不动）

负责人 Aron Rebuild 再打回：每座头像旁 **常驻** 一把木槌；**相信=槌不动**；**质疑=该放置点就地抬砸→回搁**。人模静态贴图 **零改**。全员同轨（F2）。**分锚表 / 尺寸闸 / TOP·SELF 数字 / 层序 / 调试 = [17a v0.2.2](./17a-局内桌槌表现规格.md)**（对齐策划 **S19-1～12**，**勿另造短句 key**）。
...
| 锚 | **self / p1 / p2 / p3 各写各的**（认 17a §2 / §2.2）。左座=头像右侧空（朝桌心）；右座=头像左侧空（朝桌心）；对家=头像**左空/侧空且离脸**（槌右缘在 44vp 脸左外缘外 8～16vp；建议 DX −120～−128）；自机=**贴底座金框头像旁**（40vp 脸槽；底带内；右旁或上旁；禁上漂桌心、禁贴屏左缘）。**禁止**一套坐标四座复用。**废**「环–手带 / 二键簇上沿外 8～16vp」主锚 |
...
| 挂点 | 无徽只改左上字 → S19-1/2。**对家槌盖脸** / **自机槌飘桌心** → **S19-10**。自机压手 → **S19-12**。锚进手牌行 → S19-5/7/12。砸飞出放置点 / 砸点贴牌心 → S19-9。相信还在动 → S19-8。**槌过小看不见** / **右座无槌** / **三把全偏左** / **贴屏幕外缘** = 挂点（可交叉 S19-2/4/10/11） |
| **不动** | 入口语义 / 身前二键 / 超时=相信；`HAND_RING_GAP=24%` / 禁抬 `padB`；开牌 **1000/3000**；R6 中顶只 timer |

**不过（只抄策划 S19，不新造 key）：** **质疑无锤砸** / **相信无常驻锤** / **锤仅自己见** / **砸与搁不可辨** / **锤挡池或手** / **调试锤走了业务** / **锤压在牌上** / **相信还在动** / **砸点贴牌心** / **槌不在空位** / **压牌**。见 §8 F17-31～41。本版挂点：**对家槌盖脸** / **自机槌飘桌心**（交叉 S19-10/12）。

**软挂点：** `HOLD_MS=1000` 已在 0.8～1.2s 窗内。`MALLET=112` 过尺闸。`TOP_ANCHOR_DX=-56` 盖脸、`SELF_ANCHOR_DY=-96` / fallback `X=56` `DY=-168` 飘离底座头像 = 软挂点。跟锚另票 `feat/client`。本票 **零 ets**。
...
| `lb_str_hammer_not_in_blank` | 槌不在空位 | **S19-10**（F17-40；含对家槌盖脸 / 自机槌飘桌心） |
...
| F17-40 | **槌不在空位**（S19-10 `lb_str_hammer_not_in_blank` · 17a） | 含 **对家槌盖脸**、**自机槌飘桌心** / 不在底座头像旁；亦含过小看不见、右座无槌 |
...
- [ ] @LiarBar测试 F17-1～25 可抄（含 **真假当入口** / **相信仍揭牌**=S18-4 / **相信后仍不可出**=S18-14 / **质疑未揭**=S18-15 / **入口仍真假过**=S18-13 / **质疑入口看不见** / **能跳过出牌**）；F17-31～41 **只抄 S19-1～12**（锤另票细验；本版盯对家盖脸 / 自机飘桌心）；合入 ≠ 终验  
...
| **v0.3.10** | 2026-09-21 | **桌槌再打回（对齐 17a v0.2.2）**：§3.9 TOP=左空离脸；SELF=贴底座头像旁；交叉 S19-10/12（对家槌盖脸 / 自机槌飘桌心 / 压手）；F17-40/41。左/右座朝桌、96～120（client 112）、入口语义 / GAP=24% / 禁抬 padB / 开牌 1000/3000 / R6 **一字不改**。零 ets |
...
| 2026-09-21 | Aron Rebuild 再打回 + 截图：对家槌盖脸；自机槌飘离金框 | v0.3.10 §3.9 同步 [17a](./17a-局内桌槌表现规格.md) v0.2.2；F17-40/41=S19-10/12 | UI 规格岗 | **本 PR 写入 · 待会签** |

---

*维护人：UI 文档岗 · 2026-09-21 · docs only · 第2节入口会签草案 **v0.3.10** · 桌槌交叉 **17a v0.2.2**（TOP 离脸 / SELF 贴底座头像旁 / 96～120vp）/ **S19-1～12** · **质疑\|相信** · **R2** · **R5** 本手可见 · **R6** 法官句左上 · F17-28/29/30=**S18-26～28** · 交叉 18 **v0.1.6** 1000/3000 · 靶=池 · 会签前零 ets · 合入 ≠ 终验*
