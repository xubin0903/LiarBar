...
|----|------|
| 版本 | v0.1.0 |
| 状态 | **第2节入口会签草案**（PM）· 线框闸 · 未会签禁止落 ets |
...
6. **揭牌门** — 点假之后必须权威 ack 才翻；禁止本地先开（第3节写呈现）。
...
| 4 | **真 / 假** | **同层两颗钮**，禁止嵌套（先点「质疑」再拆真假） |
| 5 | **过** | 静默；**禁止**强制过 SFX |
| 6 | **超时** | 推荐 **10s**（窗 **8～12**）。倒计时环 **可选**（UI 可带环出货） |
| 7 | **揭牌门** | **ack 后翻**；禁止本地先开。第3节写呈现；本线只锁门 |
...
AwaitChallenge → 真 / 过 / 超时 → 当前座出牌（认 15）
AwaitChallenge → 假 → 等 ack → 第3节揭牌（本线只锁门）
...
stateDiagram-v2
  ...
  AwaitChallenge --> OwnPlay: 真 / 过 / 超时
  AwaitChallenge --> ChallengePendingAck: 假 · 等 ack
...
| **ChallengePendingAck** | 已发质疑，等权威 | 入口应收或 `_disabled`；**牌背仍扣** | **禁止**本地先翻。ack 后才进第3节 |
...
点 **假** → 发 `INTENT_CHALLENGE` → **等权威 ack** → 第3节才翻上家这一手。

| 要 | 不要 |
|----|------|
| ack **接受** 之后才进第3节揭牌 | 点假立刻本地翻 `lb_cmp_play_front_pile` / `lb_cmp_reveal_card_*` |
...
确认条与质疑入口 **不得同时可点**（R-LAND-4：稳态无大双 CTA）。`AwaitChallenge` 时确认条收起；真/过之后入口卸掉，选牌后再出 15 确认条。
...
| 钮 | 语义 | SFX | 下一拍 |
|----|------|-----|--------|
| `lb_btn_challenge_true` | 认上家本手为真；**不揭牌** | `lb_sfx_challenge_commit` | 卸入口；当前座进入自己的出牌（15） |
| `lb_btn_challenge_false` | 认定假，发起质疑 | `lb_sfx_challenge_commit` | 等 ack → 第3节；禁本地先开 |
| `lb_btn_challenge_pass` | 过 | **禁止**强制过 SFX | 等同超时：卸入口，不质疑 |
...
| `lb_btn_challenge_true` | 按钮 | 同上 | **真**。与假同层 |
| `lb_btn_challenge_false` | 按钮 | 同上 | **假**。与真同层。点后等 ack |
...
静默局字幕仍出本句。  
15 客户端预检未发出质疑的，不改叫本 key。本 key 只用于 **已经点假（或等价提交）之后** 的权威拒。
...
- [ ] @LiarBar负责人 入口闸 / 真假同层 / 10s / ack 后翻 / 禁双轨  
...
- [ ] @LiarBar鸿蒙 `AwaitChallenge` 对表；飞中零钮；假→等 ack；双槽 SFX；拒只认 `lb_str_challenge_reject`；会签前零 ets  
- [ ] @LiarBar测试 F17-1～10 可抄；F16-1～12 不改号；合入 ≠ 终验；禁止空 OK  
...
*维护人：UI 文档岗 · 2026-09-13 · docs only · 第2节入口会签草案 · 会签前零 ets · 合入 ≠ 终验*
