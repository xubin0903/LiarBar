...
| 20 | **桌槌表现（v0.3.9 · 旁挂 17a v0.2.1）** | 四座常驻；质疑=就地砸回搁；相信=槌不动。分锚 / 尺 / 层序 / 调试认 **[17a](./17a-局内桌槌表现规格.md) v0.2.1**（空白带=头像与桌心之间；**废硬外偏**；self / p1 / p2 / p3 **各写各的**；常驻 **96～120vp**）。入口语义、身前二键、超时=相信、GAP=24%、禁抬 `padB`、开牌 1000/3000、R6 中顶只 timer **不动** |
...
### 3.9 桌槌表现（v0.3.9 · 线框认 17a v0.2.1 · 入口语义不动）

负责人 Aron Rebuild 纠锁：每座头像旁 **常驻** 一把木槌，落在 **头像与桌心之间的空白带**（**废硬外偏 / 贴屏幕外缘**）；**相信=槌不动**；**质疑=该放置点就地抬砸→回搁**。人模静态贴图 **零改**。全员同轨（F2）。**分锚表 / 尺寸闸 / 层序 / 调试 = [17a v0.2.1](./17a-局内桌槌表现规格.md)**（对齐策划 [#243](https://github.com/xubin0903/LiarBar/pull/243) / **S19-1～9**，**勿另造短句 key**）。

| 锁 | 数字 / id |
|----|-----------|
| 层 | `lb_cmp_choice_act`：全员桌面层；`HitTest None`；四座常驻 |
| 正式 ACK | 质疑座播砸；相信座 **保持静置可见即可**（不播搁动画） |
| 调试 | 立刻只播 **本地玩家座位**（质疑=抬砸；相信=确认静置零位移）；**零业务** |
| 锚 | **self / p1 / p2 / p3 各写各的**（认 17a §2 分表）。槌落 **头像与桌心之间的空白带**。p3=头像右侧空（朝桌心，禁贴屏左缘/挖孔）；p1=头像左侧空（朝桌心，禁贴屏右缘外侧）；p2=上沿旁空；self=旁空且不压手牌。**禁止**一套坐标四座复用。**废**「环–手带 / 二键簇上沿外 8～16vp / 硬外偏 / 左左外 / 右右外」主锚 |
| 尺 | 常驻显示边长 **96～120vp**（认 17a §2.1）。**72** 缩略主读 = 不过。抬砸同画幅 |
| 质疑=砸 | 该座放置点 **就地抬起→砸下**（短位移）→回搁。砸点=放置点附近空白。**禁飞牌心 / 池心** |
| 相信=静置 | **槌不动**。废「相信也播搁置动画」 |
...
| 挂点 | 无徽只改左上字 → S19-1/2。锚压头像 → S19-5。锚进手牌行 → S19-5/7。砸飞出放置点 / 砸点贴牌心 → S19-9。相信还在动 → S19-8。**槌过小看不见** / **右座无槌** / **三把全偏左** / **贴屏幕外缘** = 挂点口径（不另造 `lb_str_*`；可交叉 S19-2/4/7） |
| **不动** | 入口语义 / 身前二键 / 超时=相信；`HAND_RING_GAP=24%` / 禁抬 `padB`；开牌 **1000/3000**；R6 中顶只 timer |

**不过（只抄策划 S19-1～9，不新造 key）：** **质疑无锤砸** / **相信无常驻锤** / **锤仅自己见** / **砸与搁不可辨** / **锤挡池或手** / **调试锤走了业务** / **锤压在牌上** / **相信还在动** / **砸点贴牌心**。见 §8 F17-31～39。本版四句只挂点：**槌过小看不见** / **右座无槌** / **三把全偏左** / **贴屏幕外缘**。

**软挂点：** `HOLD_MS=1000` 已在 0.8～1.2s 窗内。现帧 `MALLET_W/H=72` 与贴屏外缘落点 = 软挂点。`choiceActPosOf` 若仍 `SELF_SIDE_VP` + 朝池 `INWARD_PCT` + 相信 rest 位移 = 软挂点。本票 **零 ets**。
...
| 另造 S19 短句 key | 只抄策划 #243 **S19-1～9** |
...
| F17-34 | **砸与搁不可辨**（S19-4 `lb_str_choice_act_same_pose`） | 抬砸与搁置姿难辨 |
...
- [ ] @LiarBar测试 F17-1～25 可抄（含 **真假当入口** / **相信仍揭牌**=S18-4 / **相信后仍不可出**=S18-14 / **质疑未揭**=S18-15 / **入口仍真假过**=S18-13 / **质疑入口看不见** / **能跳过出牌**）；F17-31～39 **只抄 S19-1～9**（锤另票细验）；合入 ≠ 终验  
...
| **v0.3.9** | 2026-09-21 | **桌槌纠锁（对齐 17a v0.2.1）**：§3.9 空白带=头像与桌心之间；**废硬外偏**；p3 头像右侧空 / p1 头像左侧空 / p2 上沿旁空 / self 旁空不压手；常驻 **96～120vp**；挂点 槌过小看不见 / 右座无槌 / 三把全偏左 / 贴屏幕外缘（不另造 key）。入口语义 / GAP=24% / 禁抬 padB / 开牌 1000/3000 / R6 **一字不改**。零 ets |
| **v0.3.8** | 2026-09-19 | **桌槌重钉（对齐 17a v0.2.0 / 策划 #243）**：§3.9 四座常驻分锚 self/p1/p2/p3；相信=槌不动；质疑=就地砸回搁；F17-31～39 = **S19-1～9**（不另造 key）。入口语义 / GAP=24% / 禁抬 padB / 开牌 1000/3000 / R6 **一字不改**。零 ets。**「外侧」读法已被 v0.3.9 / 17a v0.2.1 SUPERSEDE** |
...
| 2026-09-19 | 负责人 Aron Rebuild 重钉：四座常驻分锚；相信=槌不动；就地砸；S19-7～9 | v0.3.8 §3.9 同步 [17a](./17a-局内桌槌表现规格.md) v0.2.0 / #243 | UI 规格岗 | 已合 #247（「外侧」读法已被 v0.3.9 / 17a v0.2.1 SUPERSEDE） |
| 2026-09-21 | Aron 真机纠锁：空白带≠硬外偏；槌过小；右座无槌；三把全偏左 | v0.3.9 §3.9 同步 [17a](./17a-局内桌槌表现规格.md) v0.2.1 | UI 规格岗 | **本 PR 写入 · 待会签** |

---

*维护人：UI 文档岗 · 2026-09-21 · docs only · 第2节入口会签草案 **v0.3.9** · 桌槌交叉 **17a v0.2.1**（空白带 / 废硬外偏 / 96～120vp）/ **S19-1～9** · **质疑\|相信** · **R2** · **R5** 本手可见 · **R6** 法官句左上 · F17-28/29/30=**S18-26～28** · 交叉 18 **v0.1.6** 1000/3000 · 靶=池 · 会签前零 ets · 合入 ≠ 终验*
