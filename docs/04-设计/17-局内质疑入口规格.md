...
| 20 | **桌槌表现（v0.3.8 · 旁挂 17a v0.2.0）** | 四座常驻；质疑=就地砸回搁；相信=槌不动。分锚 / 层序 / 调试认 **[17a](./17a-局内桌槌表现规格.md) v0.2.0**（self / p1 / p2 / p3 **各写各的**）。入口语义、身前二键、超时=相信、GAP=24%、禁抬 `padB`、开牌 1000/3000、R6 中顶只 timer **不动** |
...
### 3.9 桌槌表现（v0.3.8 · 线框认 17a v0.2.0 · 入口语义不动）

负责人 Aron Rebuild 重钉：每座头像旁 **常驻** 一把木槌；**相信=槌不动**；**质疑=该放置点就地抬砸→回搁**。人模静态贴图 **零改**。全员同轨（F2）。**分锚表 / 层序 / 调试 = [17a v0.2.0](./17a-局内桌槌表现规格.md)**（对齐策划 [#243](https://github.com/xubin0903/LiarBar/pull/243) / **S19-1～9**，**勿另造短句 key**）。
...
| 锚 | **self / p1 / p2 / p3 各写各的**（认 17a §2 分表）。每座头像旁 **空白带**。**禁止**一套坐标四座复用。**废**「环–手带 / 二键簇上沿外 8～16vp」主锚 |
...
| 挂点 | 无徽只改左上字 → S19-1/2。锚压头像 → S19-5。锚进手牌行 → S19-5/7。砸飞出放置点 / 砸点贴牌心 → S19-9。相信还在动 → S19-8 |
| **不动** | 入口语义 / 身前二键 / 超时=相信；`HAND_RING_GAP=24%` / 禁抬 `padB`；开牌 **1000/3000**；R6 中顶只 timer |

**不过（只抄策划 S19-1～9，不新造 key）：** **质疑无锤砸** / **相信无常驻锤** / **锤仅自己见** / **砸与搁不可辨** / **锤挡池或手** / **调试锤走了业务** / **锤压在牌上** / **相信还在动** / **砸点贴牌心**。见 §8 F17-31～39。

**软挂点：** `HOLD_MS=1000` 已在 0.8～1.2s 窗内。`choiceActPosOf` 若仍 `SELF_SIDE_VP` + 朝池 `INWARD_PCT` + 相信 rest 位移 = 软挂点。本票 **零 ets**。
...
| **v0.3.8** | 2026-09-19 | **桌槌重钉（对齐 17a v0.2.0 / 策划 #243）**：§3.9 四座常驻分锚 self/p1/p2/p3；相信=槌不动；质疑=就地砸回搁；F17-31～39 = **S19-1～9**（不另造 key）。入口语义 / GAP=24% / 禁抬 padB / 开牌 1000/3000 / R6 **一字不改**。零 ets |
...
| 2026-09-19 | 负责人 Aron Rebuild 重钉：四座常驻分锚；相信=槌不动；就地砸；S19-7～9 | v0.3.8 §3.9 同步 [17a](./17a-局内桌槌表现规格.md) v0.2.0 / #243 | UI 规格岗 | **本 PR 写入 · 待会签** |

---

*维护人：UI 文档岗 · 2026-09-19 · docs only · 第2节入口会签草案 **v0.3.8** · 桌槌交叉 **17a v0.2.0** / **S19-1～9** · **质疑\|相信** · **R2** · **R5** 本手可见 · **R6** 法官句左上 · F17-28/29/30=**S18-26～28** · 交叉 18 **v0.1.6** 1000/3000 · 靶=池 · 会签前零 ets · 合入 ≠ 终验*
