...
> **Supersede（2026-09-14 / #160 → [19](./19-局内轮胜与重发规格.md)）：** 本文凡写「`PlayLanded` 后 hand==0 → **RankSettle / 占名次**」的旧 L7 句（含 §0.2 #7、拍表 RankSettle 分流、§6.3 闸）**已被 [19](./19-局内轮胜与重发规格.md) RoundWin→RedealAll 覆盖**。落地后 hand==0：**仍禁止** `AwaitChallenge`（F16-12 **打光仍进质疑** 锁词有效）；下一拍 = **RoundWin tip → RedealAll**，**不是**整局 RankSettle（除非重发后仅剩 1 人有命）。落池 / 320ms / `padB` / GAP / F16-1～15 其余 **一字不改**。
...
- **Supersede：** 上句「RankSettle / 占名次」旧目的地 → 认 [19](./19-局内轮胜与重发规格.md) **RoundWin→RedealAll**（#160）。**禁 AwaitChallenge** 不变（F16-12）。
...
| v0.2.0-note19 | 2026-09-14 | **Supersede 注（不改落池正文）：** hand==0 旧「→ RankSettle」目的地改认 [19](./19-局内轮胜与重发规格.md) RoundWin→RedealAll；F16-12 禁质疑仍有效；320ms / 池 / padB / GAP **不动**。零 ets |
