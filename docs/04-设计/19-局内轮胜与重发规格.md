> **SUPERSEDED / 冻结（2026-09-14 · #164）**  
> 规则真源已迁至 **[左轮对局-状态机](../02-游戏设计/左轮对局-状态机.md) v1.1.1**。  
> **废** RoundWin→RedealAll / 3 命主胜负（#160/#162/#163）。打光 UI 主读改认 **[20 · 局内打光目标二选规格](./20-局内打光目标二选规格.md) v0.1.0**（≥3 你/上家；=2 强制质疑）。  
> 下文仅作历史 RoundWin tip / RedealAll 对照；**不得**再当引擎或 UI 主读。合入 ≠ 终验 · 会签前零 ets。

# 19 · 局内轮胜与重发规格（RoundWin → RedealAll）

| 项 | 内容 |
|----|------|
| 版本 | v0.1.0（**SUPERSEDED / 冻结** → 左轮 v1.1.1 + [20](./20-局内打光目标二选规格.md)） |
| 状态 | **冻结** · 历史可读 · 原「轮胜提示 + 全员重发会签草案」已被 #164 废止 · **合入 ≠ 终验** · 未会签禁止落 ets |
| 范围 | ~~`lb_scr_table` 上 `PlayLanded ∧ hand==0` 之后：`RoundWin` 提示 overlay → `RedealAll` 过场~~（**现行打光认 20 + 左轮 HandEmptyGate**） |
| 读者 | UI、美术、策划、测试、鸿蒙、**@LiarBar负责人** |
| 对照 | [左轮对局-状态机](../02-游戏设计/左轮对局-状态机.md)（**现行真源**）· [20 打光目标二选](./20-局内打光目标二选规格.md)（**现行打光 UI**）· [出牌扣牌-状态机 v0.4.0 SUPERSEDED](../02-游戏设计/出牌扣牌-状态机.md) · [16](./16-局内出牌飞出与身前扣牌规格.md) · [17](./17-局内质疑入口规格.md) · [18](./18-局内开牌区规格.md) · [02 命名](./02-组件与资源命名约定.md) |
| 本 PR | 顶栏 SUPERSEDED；正文保留历史对照。**零 ets** |

> **UI 管（历史）**：轮胜 tip 位 / 时长 / 不可跳过、`RedealAll` 过场挂点、淘汰座视觉口径、`lb_*`、可测失败短句。  
> **现行**：打光 ≥3/=2 认 [20](./20-局内打光目标二选规格.md)；CollectRedeal / RoundSafeWait 认左轮。失败短句 **RoundWin仍主读** = 仍以本文当主路径。  
...
| 层 | 本文做什么 | 本文不做什么 |
|----|------------|--------------|
| 拍 | ~~`PlayLanded` 后 hand==0 → **RoundWin** → **RedealAll**~~（**冻结**） | **不改** 16 飞 320ms / 落池 / 露边；**不改** 17 真假同层；**不改** 18 抽→翻 |
| 旧 L7 | 写死：**废止**「hand==0 → RankSettle / 坐穿不再出」 | 16 正文里旧 RankSettle-on-empty / F16-12「应 RankSettle」句 = **已被本篇 supersede**（见 §0.3）；本篇自身又被左轮 + 20 supersede |
...
2. **打光路径（历史）** — `PlayLanded ∧ hand==0` → **RoundWin（必须有提示）→ RedealAll**；**禁止**进 `AwaitChallenge`；**禁止**把打光当整局 RankSettle（除非 RedealAll 后已仅剩 1 人有命）。  
...
> **2026-09-14 冻结注：** 上表硬闸 1–2 中的「3 命 / RoundWin→RedealAll」已被 #164 **废止**。现行打光 = 左轮 HandEmptyGate + [20](./20-局内打光目标二选规格.md)。

### 0.2 PM 已锁摘要（必须写死 · 对齐 #160 · **历史**）
...
| 文档 | 仍有效 | 本篇 supersede |
|------|--------|----------------|
| **16** | 落点=`lb_cmp_pool`；飞 320ms；乐观飞；禁抬 `padB`；GAP=24%；F16-11 池堆先成形；hand==0 **禁** AwaitChallenge（F16-12 锁词「打光仍进质疑」**仍有效**） | 16 文中「hand==0 → RankSettle / 占名次」旧句 → 改为认本篇 **RoundWin→RedealAll**（见 16 文首 / §6.3  supersede 注）；**再被** 左轮 + 20 接管 |
| **17** | `PlayLanded ∧ hand>0 ∧ 非第一手` → AwaitChallenge；靶=池；真≠过；身前入口；tip 迁出左上 | hand==0 **零入口** 不变；目的地从「RankSettle」改为本篇 RoundWin 路径（现行再改 20） |
| **18** | ack 后 reveal_stage / 160ms / draw·flip | **不改**。打光路径 **不进** 18（现行：质疑你 / =2 强制揭才进） |

**写死：** 16 的 land-to-pool / 飞窗 / 露边 **一字不改**。本篇只接管 empty-hand 后半拍（**现已冻结，改认 20**）。

---

## 1–9 · 历史正文（保留对照 · 非现行）

> 以下 §1～§9 为 v0.1.0 历史会签稿全文结构保留位。完整拍表 / 线框 / F19-1～6 / 会签栏仍以下方原节为准；**实现与验收不得再按 RoundWin 主路径落 ets**。现行失败短句交叉见 [20 §6](./20-局内打光目标二选规格.md) **RoundWin仍主读**。
...
| id | 类型 | 说明 |
|----|------|------|
| `lb_cmp_round_win` | 组件 | RoundWin tip overlay。屏心或近宣称/池带；**禁**左上死角。**冻结：不得再作打光主读** |
| `lb_str_round_win` | 文案 key | 「本轮胜」等；并列 / 座名 soft |
| `lb_cmp_redeal_all` | 组件（可选） | RedealAll 过场层。可与左轮 CollectRedeal 表现同族复用讨论，但 **RoundWin tip 路径冻结** |
...
16 F16-1～15、17 F17-*、18 F18-1～3 **仍有效**；其中 F16-12 **打光仍进质疑** 继续管「hand==0 还进 AwaitChallenge」。本篇追加轮胜/重发句（不占 F16 号）。**现行打光另抄 [20](./20-局内打光目标二选规格.md) F20-***。
...
> **冻结后会签口径：** 上表历史会签栏不再推动 RoundWin 落码。请改会签 [20](./20-局内打光目标二选规格.md)。
...
| v0.1.0+freeze | 2026-09-14 | **SUPERSEDED / 冻结**：#164 废 RoundWin/3命；打光 UI 主读改 [20](./20-局内打光目标二选规格.md)；顶栏横幅 + 状态改冻结 |

### 审核记录

| 日期 | 意见 | 提议修改 | 提出人 | 状态 |
|------|------|----------|--------|------|
| 2026-09-14 | PM #160：打光=本轮胜→RedealAll；tip 1.0s 不可跳过 | 新建 19 v0.1.0 | UI 文档岗 | 已被 #164 覆盖 |
| 2026-09-14 | #164 废 RoundWin；打光改 ≥3/=2 | 顶栏 SUPERSEDED；交叉 20 | UI 文档岗 | **冻结** |

---

*维护人：UI 文档岗 · 2026-09-14 · docs only · 轮胜与重发 v0.1.0 · **SUPERSEDED / 冻结** · 会签前零 ets · 合入 ≠ 终验*
