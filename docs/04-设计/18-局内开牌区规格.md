...
|----|------|
| 版本 | **v0.1.4** |
| 状态 | **第3节开牌区会签草案**（PM）· 线框数字作本版闸 · **v0.1.1 Rebuild F2**：`lb_cmp_reveal_stage` = 桌级共享 overlay / 全员可见 · **v0.1.2 R1**：开牌=真实 `lastPlayRanks` / 禁宣称假面 · **v0.1.3 R1′/R3**：空 ranks 禁瞎演；禁 hold 前 reset · **v0.1.4 R4**：`DRAW_TO_FLIP=1000ms` / `REVEAL_HOLD=5000ms`（SUPERSEDE 500/2000）· 未会签禁止落 ets |
...
| 对照 | [17 质疑入口](./17-局内质疑入口规格.md)（**质疑\|相信** · ChallengeCommit→ack · **R2 第二人可质疑第一手** · F2 全员可见）· [对局-状态机 §1.3](../02-游戏设计/对局-状态机.md)（**R1′ 开牌真牌面加严** / **R4 开牌时长** SUPERSEDE R3 500/2000 / S18-24 / S18-25 · 基线 **adb03ea0**）· [16 落池](./16-局内出牌飞出与身前扣牌规格.md)（靶 = 池上本手）· [02 命名](./02-组件与资源命名约定.md) · [11 横屏](./11-局内横屏与自适应布局.md) · [GDD §6.2 质疑](../02-游戏设计/GDD.md) · [13 声场](./13-局内声场与伴奏规格.md) |
...
| 5 | **拍** | 抽→翻 **`DRAW_TO_FLIP=1000ms`**（窗 **800～1200**）；翻后持面 **`REVEAL_HOLD=5000ms`**（窗 **4～6s**）。先 `RevealDraw` 再 `RevealFlip`，不可跳过翻；**持面满窗才允许** `resetRevealUi` / `pull`（**禁** hold 前 reset）。**SUPERSEDE** v0.1.3 的 500/2000 |
...
### 0.4 R4 开牌时长（2026-09-15 Rebuild · 写死 · SUPERSEDE R3）

**写死 · PM Rebuild 锁（基线 adb03ea0）：** 废旧抽→翻 **160ms** 作主闸；**SUPERSEDE** v0.1.3 / R3 的 `DRAW_TO_FLIP=500ms` / `REVEAL_HOLD=2000ms` 作主闸。

| 常量 | 推荐（锁） | 窗口（容差） | 禁 |
|------|------------|--------------|----|
| 抽→翻 `DRAW_TO_FLIP` | **1000ms** | **800～1200ms** | 仍锁 160 / 500ms；瞬切明牌；抽完立刻消失 |
| 翻后持面 `REVEAL_HOLD` | **5000ms** | **4～6s** | 仍锁 2000ms；持面未满就 `resetRevealUi` / `pull` / 进下一拍 |

持面满窗才允许卸舞台 / 下一拍。失败短句仍 **S18-25** `lb_str_reveal_too_fast` = **开牌过快**。
...
17 AwaitChallenge → 真 / 假 → ChallengeCommit → ChallengePendingAck
                         ...
                         └─ ack 接受 → RevealDraw（抽离池 + draw SFX）
                                      ...
                                      → 持面 REVEAL_HOLD=5000ms（满窗才 reset/pull）
                                      → 裁定呈现（猜对/猜错 / 扣命 · 句可后补）
真 ≠ 过。过 / 超时不进本区。
```
...
### 2.4 抽→翻 / 持面时长（R4 · SUPERSEDE R3 500/2000 · 废 160ms 主闸）

| 项 | 锁 |
|----|-----|
| 抽→翻 `DRAW_TO_FLIP` | 锁 **1000ms**；窗 **800～1200ms** |
| 翻后持面 `REVEAL_HOLD` | 锁 **5000ms**；窗 **4～6s** |
| 缓动 | ease-out（与 15 / 16 同族，不另造曲线名） |
| 途中 | 抽离阶段牌背；翻面后见 `art_card_a` / `_k` / `_q` / `_joker` |
| 不可跳过 | 必须走 `RevealDraw`→`RevealFlip`→**持面满窗**。ack 后瞬切明牌、只抽不翻 = **开牌看不清**；抽→翻 <800ms 或持面 <4s 就 reset/pull = **开牌过快**（S18-25） |
| 卸舞台闸 | **持面满窗**才允许 `resetRevealUi` / `pull` / 进下一拍。**禁止** hold 未满提前 reset |

短于窗下沿闪揭、长于窗上沿拖沓，评委按窗打。旧 **160ms**（120～200）与 **R3 500/2000** **不作**本版主闸。
...
| F18-6 | **开牌过快**（对局 **S18-25** `lb_str_reveal_too_fast`） | 抽→翻 <800ms（或仍锁 160/500ms 主闸）；或翻后持面 <4s 就 `resetRevealUi` / `pull`；看不清点数 |
...
- [ ] @LiarBar负责人 位 56vp / 尺 0.90× / 暗场 0.40·6vp / `DRAW_TO_FLIP=1000ms` / `REVEAL_HOLD=5000ms` / 安全距 / 双槽 SFX  
...
*维护人：UI 文档岗 · 2026-09-15 · docs only · 第3节开牌区会签草案 **v0.1.4** · R1′ 真牌面加严 · **R4** 开牌时长 1000/5000 · 会签前零 ets · 合入 ≠ 终验*
