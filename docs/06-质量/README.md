...
| 文件 | 用途 | 怎么用 |
|------|------|--------|
| [测试用例.md](./测试用例.md) | MVP 主流程用例表 | 按 ID 测：TC-01～TC-18 + **TC-19** `demo_seed` 90s 冒烟；步骤引用 `lb_*`（§0 控件表；四主屏 `lb_scr_lobby` / `table` / `challenge` / `report`；四拍=`lb_challenge_windup` / `standoff` / `reveal` / `result`；看牌 `lb_cmp_peek_mask`） |
| [课设5分钟验收勾选清单.md](./课设5分钟验收勾选清单.md) | 评委 5 分钟时间轴 | 进场前预跑 + 0:00–5:00 打勾；含 3 分钟脚本与乐趣点；种子高潮=杠精开质疑（对照 `03` §3） |
