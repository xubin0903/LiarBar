...
> 规则真源已迁至 **[左轮对局-状态机](./左轮对局-状态机.md) v1.1.3**。  
> **废** 本文扣命 / RoundWin→RedealAll / 3 命主胜负旧句（#160/#163）。  
>
> **入口文案重锁（PM 2026-09-14 · 本票）**：前台二键 = **质疑** \| **相信**（锁 `lb_str_challenge_doubt` / `lb_str_challenge_believe`；btn=`lb_btn_challenge_doubt`/`_believe`；art=`art_btn_challenge_doubt`/`_believe`；SFX 仍 enter/commit，commit 仅质疑）。  
...
| 项 | 内容 |
|----|------|
| 文档版本 | **v0.2.5**（入口 id=doubt\|believe 锁；胜负仍 **SUPERSEDED** → 左轮 **v1.1.3**） |
| 状态 | **PM reject 锁：doubt\|believe id（废 commit/trust 文案键）** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets（除 string.json） |
| 主笔 | 策划（`feat/design-*`） |
...
*维护人：策划文档岗 · 2026-09-14 · docs + string.json · v0.2.5 · 入口=质疑\|相信 · 胜负 SUPERSEDED by 左轮 v1.1.3 · 合入 ≠ 终验*
