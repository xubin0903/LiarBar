...
> 规则真源已迁至 **[左轮对局-状态机](./左轮对局-状态机.md) v1.1.0**。  
> **废** 本文扣命 / RoundWin→RedealAll / 3 命主胜负旧句（#160/#163）。  
> 下文仅作历史 UI 拍（真·假同层二键 / ack 再翻 / 10s）参考；胜负 / 开枪 / 打光门 **以左轮为准**。合入 ≠ 终验 · 会签前零 ets。

# 质疑入口 · 状态机（第2节入口锁 · AwaitChallenge → ChallengeCommit / Pass）

| 项 | 内容 |
|----|------|
| 文档版本 | **v0.2.3**（**SUPERSEDED** → 左轮 v1.1.0） |
...
| 对照 | [出牌扣牌-状态机](./出牌扣牌-状态机.md)（第1节飞出 + **v0.4.0 轮胜重发**；本文件只接入口） · [GDD §6.2 / §6.1 / G-Q6](./GDD.md) · [05 T20](../05-技术/02-回合状态机草图.md) · [13 声场](../04-设计/13-局内声场与伴奏规格.md) · [02 命名](../04-设计/02-组件与资源命名约定.md) · [16 飞出](../04-设计/16-局内出牌飞出与身前扣牌规格.md)（本线 **不改** 16 正文数字） |
| 本 PR | **只交 docs**。零 `ets` / `rawfile` / `media`。**线框数字见 17 v0.2.0 / 18 v0.1.0（#148）**，本文不发明 vp |
| 制作人摘要 | 2026-09-13 第2节「质疑阶段」入口锁 |

> **线框数字见 17 v0.2.0 / 18 v0.1.0（#148）。** 本文件不写冲突 vp；开牌区几何认 18。  
> **第1节飞出 / 轮胜重发 L7（v0.4.0）仍认** [出牌扣牌-状态机](./出牌扣牌-状态机.md)。本线 **不重开** 第4节名次细表。hand==0 **仍禁** AwaitChallenge（改走 RoundWin→RedealAll，不再走旧 RankSettle-from-empty）。  
...
| **AwaitChallenge** | `TURN`（下家） | **池上该手堆保持**；**真·假同层二键**；倒计时环可选 | 点真 / 假（都 ChallengeCommit）；或过（沉默） | 第一手进来；hand==0 进来；旧四拍/弱 CTA 并行；问卷式多步；**真当过** |
...
| Q03 | PlayLanded | `PILE_FORMED` | 权威已接受 **且** `hand.count > 0` **且** 非第一手 **且** `lastPlay.actor` 本手未打光（未进 RoundWin） | **AwaitChallenge** | 播 `lb_sfx_challenge_enter`（非静默）。开 **10s** 窗。出真·假同层二键 |
...
时机 **不改**：仍 = 上家本手 PlayLanded 后进 AwaitChallenge（**非**四人出完才质疑）。真/假都 ChallengeCommit→ack→`lb_cmp_reveal_stage`。**真 ≠ 过**。

### 11b.1 荷官进质疑句（写死）

| key | 默认中文 | 何时 |
|-----|----------|------|
| `lb_str_challenge_dealer_enter` | **轮到质疑了——真，还是假？** | 进 AwaitChallenge 同起（可与 enter SFX 同窗） |
| （备选短，不另开 key） | 上家这手，信不信？ | 仅文案 A/B；默认仍用长句 |

**禁止**提示仍占左上死角（= **提示藏死角**）。宣称 / 轮到你 / 进质疑句可读区认 17 v0.2.1（#155）。

### 11b.2 当前质疑者 · 真假键

- 真人：键挂 **当前质疑者身前**（环–手带，对齐 17）；造型可辨（美术 #157 `art_btn_challenge_true` / `_false`）。
...
| key | 中文 | 何时 |
|-----|------|------|
| `lb_str_npc_challenge_thinking` | **选择真/假…** | NPC 座旁气泡起势（思考） |
| `lb_str_npc_challenge_true` | **选了真** | NPC 落定真 |
| `lb_str_npc_challenge_false` | **选了假** | NPC 落定假 |
...
| **S17-14** | **质疑入口看不见** | AwaitChallenge 无真假键，或键在死角/边角不可点 |
| **S17-15** | **提示藏死角** | 荷官进质疑句 / 宣称 / 轮到你仍压左上不可读 |
| **S17-16** | **NPC 抉择无反馈** | NPC 质疑无座旁「选择真/假…」→「选了真/假」 |
...
*维护人：策划文档岗 · 2026-09-14 · docs only · v0.2.3 · **SUPERSEDED by 左轮** · 会签前零 ets · 合入 ≠ 终验*
