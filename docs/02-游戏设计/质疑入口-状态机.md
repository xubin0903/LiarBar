> **SUPERSEDED（胜负 / 打光 / 3命 · 2026-09-14）**  
> 规则真源已迁至 **[左轮对局-状态机](./左轮对局-状态机.md) v1.1.2**。  
> **废** 本文扣命 / RoundWin→RedealAll / 3 命主胜负旧句（#160/#163）。  
>
> **入口文案重锁（PM 2026-09-14 · 本票）**：前台二键 = **质疑** \| **相信**（`lb_str_challenge_commit` / `lb_str_challenge_trust`）。  
> **旧「真·假同层二键」作入口主读 → SUPERSEDED。** 真/假 **仅**揭牌裁定结果（谁开枪），不作入口按钮。  
> 下文历史拍（ack 再翻 / 10s / enter·commit SFX）仍可参考；**入口标签与荷官句以 §0.5 + 左轮 §1.1 为准**。合入 ≠ 终验 · 会签前零 ets（本票可改 `string.json`）。

# 质疑入口 · 状态机（第2节入口锁 · AwaitChallenge → ChallengeCommit / Trust）

| 项 | 内容 |
|----|------|
| 文档版本 | **v0.2.4**（入口文案重锁；胜负仍 **SUPERSEDED** → 左轮 **v1.1.2**） |
| 状态 | **PM 锁：质疑\|相信（废真假主读）** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets（除 string.json） |
| 主笔 | 策划（`feat/design-*`） |
...
| 对照 | [左轮对局-状态机](./左轮对局-状态机.md)（**入口真源**） · [出牌扣牌-状态机](./出牌扣牌-状态机.md)（历史飞出） · [GDD §6.2](./GDD.md) · [05 T20](../05-技术/02-回合状态机草图.md) · [17](../04-设计/17-局内质疑入口规格.md) |
| 本 PR | **docs + `entry/.../string.json`**。零 `ets` / `rawfile` / `media`。 |
| 制作人摘要 | 2026-09-14 入口废真/假主读 → **质疑\|相信** |
...
| 键 | 语义 | 结果 | lb_str_* |
|----|------|------|----------|
| **质疑** | ChallengeCommit：揭上家本手 | ack 接受 → Reveal | `lb_str_challenge_commit` = 质疑 |
| **相信** | Trust / Pass：不揭，续对 | → 下家可继续出牌 | `lb_str_challenge_trust` = 相信 |
...
6. 建议控件 id：`lb_btn_challenge_commit` / `lb_btn_challenge_trust`（旧 `lb_btn_challenge_true` / `_false` 不得再当入口主读）。
...
| 建议 id | 含义 |
|---------|------|
| `lb_btn_challenge_commit` | 质疑（新） |
| `lb_btn_challenge_trust` | 相信（新） |
...
| `lb_str_challenge_commit` | **质疑** | 入口键 |
| `lb_str_challenge_trust` | **相信** | 入口键 |

**禁止**提示仍占左上死角（= **提示藏死角**）。宣称 / 轮到你 / 进质疑句可读区认 17 v0.2.1（#155）。

### 11b.2 当前行动者 · 质疑/相信键

- 真人：键挂 **当前行动者身前**（环–手带，对齐 17）；造型可辨（建议 `art_btn_challenge_commit` / `_trust`；旧 true/false 美术可暂映射但文案必须质疑/相信）。
...
| `lb_str_npc_challenge_commit` | **选了质疑** | NPC 落定质疑（新键） |
| `lb_str_npc_challenge_trust` | **选了相信** | NPC 落定相信（新键） |
...
*维护人：策划文档岗 · 2026-09-14 · docs + string.json · v0.2.4 · 入口=质疑\|相信 · 胜负 SUPERSEDED by 左轮 v1.1.2 · 合入 ≠ 终验*
