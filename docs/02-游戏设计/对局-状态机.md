...
|----|------|
| 文档版本 | **v2.0.4** |
| 状态 | **2026-09-14 PM/Aron 锁：废左轮；lives_default=3 烛；揭牌输家直接熄1烛；熄尽淘汰；末存活胜** · **F2 全员可见** · **R1/R2** · **R1′开牌真牌面加严 / R3开牌时长（2026-09-15 Rebuild再打回）** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets |
...
> **R3 开牌时长（2026-09-15 Rebuild再打回）**：抽→翻 **`DRAW_TO_FLIP=500ms`**（窗 **400～600ms**）；翻后持面 **`REVEAL_HOLD=2000ms`**（窗 **1.5～2.5s**）；持面满窗才允许 reset/pull。**废**旧抽→翻 160ms 作主闸。交叉 [18](../04-设计/18-局内开牌区规格.md)。  
...
| 拍 | 锁值 | 窗 | 禁 |
|----|------|----|----|
| 抽→翻 `DRAW_TO_FLIP` | **500ms** | **400～600ms** | 旧 160ms 主闸；瞬切 / 抽完立刻消失 |
| 翻后持面 `REVEAL_HOLD` | **2000ms** | **1.5～2.5s** | 持面未满就 reset/pull / 进下一拍 |

交叉 [18](../04-设计/18-局内开牌区规格.md)（UI 跟数字）；失败短句 **S18-24**（开牌假面）/ **S18-25**（开牌过快）。
...
| **S18-25** | **开牌过快** | `lb_str_reveal_too_fast` | 抽→翻 <400ms（或仍锁 160ms 主闸）；或翻后持面 <1.5s 就 reset/pull；看不清点数 |
