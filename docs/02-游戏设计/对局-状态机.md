...
|----|------|
| 文档版本 | **v2.0.8** |
| 状态 | **仅附录E，正文规则零改动** · 废左轮 · 3烛 · F2 · R1/R2/R1′ · R4开牌时长 1000/3000 · R5质疑窗本手张数可见 · R6法官句全左上/中顶只timer · 会签草案 · **合入 ≠ 终验** |
...
> **R4 开牌时长（2026-09-15 · 改数）**：抽→翻 **`DRAW_TO_FLIP=1000ms`**（窗 **800～1200ms**）；翻后持面 **`REVEAL_HOLD=3000ms`**（窗 **2.5～3.5s**）；持面满窗才 reset/pull。**SUPERSEDE** 3500 / 5000 / 2000 / 160ms 主闸。交叉 [18](../04-设计/18-局内开牌区规格.md)。  
...
**写死 · 时长（R4 · SUPERSEDE 3500）：**

| 拍 | 锁值 | 窗 | 禁 |
|----|------|----|----|
| 抽→翻 `DRAW_TO_FLIP` | **1000ms** | **800～1200ms** | 仍锁 160/500ms；瞬切 / 抽完立刻消失 |
| 翻后持面 `REVEAL_HOLD` | **3000ms** | **2.5～3.5s** | 仍锁 3500/5000/2000ms；持面未满就 reset/pull / 进下一拍 |
...
| **S18-25** | **开牌过快** | `lb_str_reveal_too_fast` | 抽→翻 <800ms（或仍锁 160/500ms）；或翻后持面 <2.5s / 仍锁 3500/5000ms 主读就 reset/pull；看不清点数 |
...
| **v2.0.6** | **改数**：`REVEAL_HOLD=3500ms`（窗 3～4s；SUPERSEDE 5000）；S18-25 阈值跟新窗；对齐 #204 |
| **v2.0.7** | **改数**：`REVEAL_HOLD=3000ms`（窗 2.5～3.5s；SUPERSEDE 3500）；S18-25 阈值跟新窗；string 不动（client 已跟） |
| **v2.0.8** | **仅附录 E**：引擎事件契约映射表（供 Cocos 移植 / MatchEvents）；**正文规则零改动**；事件不新增规则 |

## 附录 E · 引擎事件契约映射表（v2.0.8）

> **目的**：把正文已锁主链 / U 表 / R1–R6 / HandEmptyGate / PenaltyExtinguish1 / CollectRedeal / 末存活胜，映射为引擎可 `drain(seq)` 的事件名，供 Cocos TS 一比一移植与 `engine_parity` 对照。  
> **锁**：**事件不新增规则**；不改胜负、生命、质疑入口、开牌真牌面、时长数字。字段语义必须能回指正文 U 行 / §2.6 / §1.x；冲突时以正文为准。  
> **工程对照（只读）**：文档阶段名 ↔ `Phase.ets` / `TurnWindow` 不完全同名（见术语表）；对齐时以本表 + U 表为准，勿死抠字符串相等。草表来源：`08-status/05-engine-refactor-plan.md` 事件草表。

| 事件 | 阶段（文档） | 条件 | 字段 | 表现要求 | S18 |
|------|--------------|------|------|----------|-----|
| `MatchStarted` | MATCH → Deal5（U01） | 开局/新局进入发牌前 | `matchId`, `seed`, `livesDefault=3` | 开局每人 **3 烛**；无左轮膛位主读 | S18-20 |
| `Dealt` | Deal5（U02） | 座位发满本轮手牌 | `seat`, `count`（=5） | 每人发 5；牌堆构成 20=6K+6Q+6A+2Joker | S18-7 / S18-3 |
| `ClaimSet` | Designate（U03） | 本轮宣称已公布 | `rank`（K/Q/A） | 中顶**不**叠宣称主读；法官/荷官句左上（R6） | S18-27 / S18-28 |
| `TurnBegan` | Play / TURN | 轮到该座行动窗打开 | `seat`, `window`（NORMAL / EMPTY_SAFE / FORCE_CHALLENGE / …） | 中顶仅 `lb_txt_timer`；出局座不得开窗 | S18-3 / S18-28 |
| `PlayLanded` | Play → TrustOrChallenge 或 HandEmptyGate（U04） | 权威接受本手提交 | `seat`, `count`(1–3), `claim` | **含本轮第一手**：hand>0 → 下家必须进质疑\|相信（R2）；hand==0 → EmptyGate | S18-6 |
| `ChallengeWindowOpened` | TrustOrChallenge / AwaitChallenge（U04/U20） | 上家本手已落地且 hand>0（含第一手） | `seat`, `prevSeat`, `count`（=`lastPlay.count`） | 入口 **质疑\|相信**；池上本手牌背可见、张数=count（R5）；荷官进窗句全员可读（F2） | S18-6 / S18-13 / S18-26 / S18-22 |
| `Believed` | TrustOrChallenge → Play（U05） | 点 **相信** / Trust | `seat` | **不**进揭牌；续对可出；无 commit 质疑 SFX | S18-4 / S18-14 |
| `ChallengeCommitted` | TrustOrChallenge → Reveal（U06） | 点 **质疑** / ChallengeCommit（ack 后） | `seat`, `targetSeat` | 全员可见进窗/气泡/揭牌门（F2）；禁本地先揭 | S18-15 / S18-22 / S18-23 |
| `RevealStarted` | Reveal / CHALLENGE_RITUAL（§1.3） | 质疑 ack 后开揭牌 | `ranks`（**真** `lastPlayRanks`）, `claim` | faceUp=真实打出牌（R1）；禁 claim 假面；抽→翻 **1000ms**、持面 **3000ms**（R4）；全员可见舞台 | S18-24 / S18-25 / S18-23 |
| `Judged` | Reveal → PenaltyExtinguish1（U07/U08） | 真/假裁定完成 | `liar`（是否假）, `loserSeat` | 真→质疑者熄1；假→上家（骗子）熄1；Joker 除外；裁定句全员可读、左上（R6） | S18-2 / S18-12 / S18-27 |
| `CandleOut` | PenaltyExtinguish1（U09/§5） | 对输家执行熄 1 烛 | `seat`, `livesLeft` | **直接熄1**；无开枪/左轮；熄灭序列全员可见 | S18-18 / S18-17 / S18-19 |
| `SeatEliminated` | → Eliminated（U11） | 该座 `livesLeft==0` | `seat` | 烛尽淘汰；此后不得发/出/质疑 | S18-21 / S18-3 |
| `EmptyGate` | HandEmptyGate（U13/U17 / §2.6） | `hand==0` 打光 | `kind`：`3`（存活≥3→EmptySafe3）\| `2`（存活==2→ForceChallenge2） | ≥3：可选「质疑你 / 不质疑你」；=2：**强制**质疑最后一手；废 RoundWin | S18-8 / S18-9 / S18-10 / S18-11 |
| `Redealt` | CollectRedeal → Designate（U09/U10/U19 / §6） | PenaltyExtinguish1 完成，或 RoundSafeWait 主/收束触发 | （可空或 `roundIndex`） | 收牌重发；**不绑枪**；存活者重发 5+新宣称；禁静默跳过 | S18-1 / S18-5 / S18-17 |
| `MatchEnded` | MatchEnd（U12） | 场上仅剩 **1** 存活（末存活胜） | `winnerSeat` | 末存活胜；禁旧左轮/RoundWin 整局公式 | S18-5 / S18-17 |

**覆盖核对（本附录 ↔ 正文锁，零新规则）：**

| 锁点 | 事件行 |
|------|--------|
| R1 开牌真牌面 | `RevealStarted.ranks` = 真出牌 |
| R2 第一手可质疑 | `PlayLanded`（含第一手）→ `ChallengeWindowOpened` |
| R4 1000/3000 | `RevealStarted` 表现要求 |
| R5 本手张数可见 | `ChallengeWindowOpened.count` |
| R6 法官左上 / 中顶只 timer | `ClaimSet` / `TurnBegan` / `Judged` 表现要求 |
| 打光 ≥3 / =2 | `EmptyGate.kind` |
| PenaltyExtinguish1 | `Judged` → `CandleOut` |
| CollectRedeal | `Redealt` |
| 末存活胜 | `MatchEnded` |
...
| 日期 | 岗 | 结论 |
|------|----|------|
| 2026-09-17 | 策划 D1 | **ACK**：附录 E 仅映射正文 U/R/§2.6；事件不新增规则；正文规则零改动（v2.0.7→v2.0.8） |
| 2026-09-17 | 策划 D2 | **与 05 §3.3 对齐**：十五事件名一一对应；草表载荷可映射至本附录；无冲突条号；正文规则仍零改 |
