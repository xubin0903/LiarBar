...
|----|------|
| 文档版本 | **v2.0.6** |
| 状态 | **废左轮 · 3烛 · F2 · R1/R2/R1′** · **R4开牌时长 1000/3500 · R5质疑窗本手张数可见 · R6法官句全左上/中顶只timer（2026-09-15）** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets |
...
> **R4 开牌时长（2026-09-15 · 改数）**：抽→翻 **`DRAW_TO_FLIP=1000ms`**（窗 **800～1200ms**）；翻后持面 **`REVEAL_HOLD=3500ms`**（窗 **3～4s**）；持面满窗才 reset/pull。**SUPERSEDE** 5000 / 2000 / 160ms 主闸。交叉 [18](../04-设计/18-局内开牌区规格.md)。  
...
**写死 · 时长（R4 · 对齐 #204 · SUPERSEDE 5000）：**

| 拍 | 锁值 | 窗 | 禁 |
|----|------|----|----|
| 抽→翻 `DRAW_TO_FLIP` | **1000ms** | **800～1200ms** | 仍锁 160/500ms；瞬切 / 抽完立刻消失 |
| 翻后持面 `REVEAL_HOLD` | **3500ms** | **3～4s** | 仍锁 5000/2000ms；持面未满就 reset/pull / 进下一拍 |
...
| **S18-25** | **开牌过快** | `lb_str_reveal_too_fast` | 抽→翻 <800ms（或仍锁 160/500ms）；或翻后持面 <3s / 仍锁 5000ms 主读就 reset/pull；看不清点数 |
