...
|----|------|
| 文档版本 | **v1.1.3** |
| 状态 | **2026-09-14 PM reject 锁：入口 id=doubt\|believe；废 commit/trust 文案键** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets |
...
7. `lb_str_challenge_true`/`false` **必须保留**（reveal/placeholder）；仅标 DEPRECATED for entry。
...
## 3. 入口 id 锁（PM reject 2026-09-14 · write dead）
...
| `lb_str_challenge_true` / `lb_str_challenge_false` | **真** / **假** | 揭牌裁定层 / placeholder | **必须保留**；`_comment` 标 DEPRECATED for entry；**不得**当入口按钮标签 |
| `lb_str_challenge_disabled_first` | **第一手还没人出过牌** | 第一手禁用入口 affordance | **不得**作局内 tip 主读（old tip-deleted lock） |
...
| **S18-16** | **左轮膛位当命数主读** | `lb_str_revolver_as_lives` | 用左轮膛数 / 子弹图标替代烛，作为「还剩几命」计数 UI |
...
| v1.1.2 | PM 锁：入口废真/假主读；前台 **质疑\|相信**；草稿文案键 commit/trust；荷官句改；S18-13～15；生命主读烛+熄灭 |
| **v1.1.3** | PM reject：锁 id=**doubt\|believe**（btn/str/art）；**RESTORE** true/false（deprecated entry）；废 commit/trust 文案键（免撞 SFX）；S18-14/15→believe/doubt 键名；disabled_first 非 tip 主读；**S18-16** `lb_str_revolver_as_lives` |
