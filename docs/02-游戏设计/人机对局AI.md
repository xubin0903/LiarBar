...
|----|------|
| 文档版本 | **v0.1.2**（会签稿） |
| 状态 | **对齐调参表 v1（回归）** · 会签草案 · **合入 ≠ 终验** · **决策真源 → [人机对局AI-核心算法](./人机对局AI-核心算法.md) v0.2.0**（三档窗 SUPERSEDE 旧 Timid Slam≈0）
| 主笔 | 策划（`feat/design`） |
| 路径 | `docs/02-游戏设计/人机对局AI.md` |
| 读者 | @LiarBar负责人 · @LiarBar鸿蒙开发 · @LiarBar测试 · @LiarBar UI |
| 对齐 | [对局-状态机](./对局-状态机.md) **v2.0.x（规则真源）** · [人机对局AI-核心算法](./人机对局AI-核心算法.md) **v0.2.0（决策真源）** · [GDD §8](./GDD.md) · [数值与牌堆配置 §5](./数值与牌堆配置.md) · [质疑入口-状态机](./质疑入口-状态机.md)（入口 id） · [AI荷官与牌友规格](../03-鸿蒙与AI/AI荷官与牌友规格.md)（话术/表演层；**决策公式主读核心算法**） |
...
### 4.4 Soft×1 / Slam×3 防塌缩锁（2026-09-21 Rebuild 打回）

**问题**：读感塌成假 AI——杠精≈必疑、另两≈必信（与 `demo_seed` 强制同构）。

**会签数（写死）**

| 窗 | 锁 | 期望分布（同 InfoSet 夹具 · n≥50 · 无噪声或噪声关） | 禁（=失败 / AI假决策） |
|----|----|------------------------------------------------------|-------------------------|
| **Soft×1** | 上家 `style=SOFT` 且弱可疑（`S_raw` 低档：无 SLAM/无大摊3/无高压史） | **三人格均多数相信**：`P(BELIEVE) ≥ 0.55`（Karen/Shark/Timid 全过） | Soft 窗仍 Karen 必疑 / Timid·Shark 必信塌缩 |
| **Slam×3** | 上家 `style=SLAM` 或强可疑（`S_raw` 高档：SLAM+count3 或高揭假史） | 质疑率写死（鸿蒙调参表 v1）：**Karen ≥70%** · **Shark 30～60%** · **Timid <20%**（同夹具 n≥50） | Slam 窗未达上列窗；或再塌成二值必疑/必信 |

**实现钉（鸿蒙应对齐，本岗锁语义）**

1. Soft 窗：`sig_style(SOFT)=0`；**禁**再对 Soft 额外加质疑偏置；`S_fake` 放大后 Soft 夹具仍须让三人格 `Score < T` 为常态（Soft×1）。
2. Slam 窗：保留 `sig_style(SLAM)=+0.18` 等；人格靠 `challengeSensitivity` + `T` 拉开；**禁**座号写死「左必疑」。
3. `challengeSensitivity` 仍认 §4.2：**Karen 0.72 / Shark 0.42 / Timid 0.18**（高/中/低）。
4. 废假 AI 路径（`demo_seed` force / 左必疑）仍锁关；本钉是**防参数读感塌缩**，不重开规则。

> **交叉**：鸿蒙调参表 **v1**（Soft×1 多信；Slam×3 K≥70 / S30～60 / T<20）为实装验收数；本岗与之一字对齐。
...
| U-AI-04b | **Soft×1** 夹具（SOFT+弱可疑） | 三人格 `P(BELIEVE)≥0.55` | Soft 仍塌成杠精必疑/另两必信 = **AI假决策** |
| U-AI-04c | **Slam×3** 夹具（SLAM/强可疑） | Karen≥70% / Shark 30～60% / Timid<20% | 未达窗或二值塌缩 = **AI假决策** |
...
| S-AI-08 | Soft×1 夹具 | 三人格多数信 | AI假决策 |
| S-AI-09 | Slam×3 夹具 | Karen≥70% / Shark 30～60% / Timid<20% | AI假决策 |
...
| v0.1.1 | 2026-09-21 | Soft×1 / Slam×3 防塌缩 |
| **v0.1.2** | 2026-09-21 | **对齐鸿蒙调参表 v1**：Slam Karen≥70% / Shark 30～60% / Timid<20%；Soft×1 仍多数信 |
...
- [ ] 失败短句键已挂或登记  
- [ ] Soft×1 / Slam×3 防塌缩验收可测；禁再塌假AI读感  
