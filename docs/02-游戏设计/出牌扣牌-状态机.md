...
```
权威成功 → PlayLanded 池上该手堆成形（飞窗必须播完）→ 然后才 RankSettle / 宣称名次
```
...
| **PlayLanded** | 本手展示刚成形 | 中央 `lb_cmp_pool` **本轮该手牌背堆已成形**（露边可读张）；HitTest None | 池叠成形后立刻按 hand 分流 | 堆未成形就进 AwaitChallenge 或 RankSettle；**落点不在牌堆**；**身前扣区仍主读** |
...
| **RankSettle** | 入名次 / 战报前展示 | 池叠成形后的名次 / 宣称名次 UI | 打光者入席次。细分配分 **第4节详** | 飞中抢切；进 AwaitChallenge；对打光者再开质疑 |
...
新手替换旧手叠。新宣称卸本轮堆主读。打光入 RankSettle 后，该手堆可留到名次展示结束；**不得**再当下游质疑靶。坟场几何 = **§2/3**。
...
| D11 | 飞中出胜利/名次；或打光仍走 §10.1 仅质疑窗 | S16-10 / 旧胜负句 | 池叠成形后 RankSettle；打光者不再出牌、不再被质疑 |
...
*维护人：策划文档岗 · 2026-09-14 · docs only · v0.3.1 落池+补表 · 会签前零 ets · 合入 ≠ 终验*
