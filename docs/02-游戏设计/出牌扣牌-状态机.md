...
|----|------|
| 文档版本 | **v0.2.0** |
| 状态 | **制作人第1节已锁** · **Aron 打光名次已锁** · 会签草案 · **合入 ≠ 终验** · 会签前零 ets |
| 主笔 | 策划（`feat/design-*`） |
| 范围 | 选牌确认之后：乐观飞出 → 身前扣牌成形 → **手牌>0 则 AwaitChallenge 入口**；**hand==0 则名次结算、禁进 AwaitChallenge**。不写第2节质疑深潜 |
| 读者 | @LiarBar负责人 · @LiarBar UI · @LiarBar测试 · @LiarBar鸿蒙开发 · @LiarBar美术 |
| 对照 | [16 飞出与身前扣牌](../04-设计/16-局内出牌飞出与身前扣牌规格.md)（**#134** 数字；跟票 **#137** 维护 16 正文）· [15 触摸出牌](../04-设计/15-局内手牌触摸出牌规格.md) · [手牌触摸出牌-交互方案](./手牌触摸出牌-交互方案.md) · [GDD §2.2 / §6.1 / §10.1](./GDD.md) · [05 T20](../05-技术/02-回合状态机草图.md) · [13 声场](../04-设计/13-局内声场与伴奏规格.md) · [02 命名](../04-设计/02-组件与资源命名约定.md) · [测试 C1](../06-质量/开桌加载-牌面-发牌验收勾选清单.md) |
...
> **质疑已解冻**；旧质疑双轨 **禁并存**。四拍 / 裁定演出 / 坟场层 = **第2节详**。细分配分表 = **第4节详**。  
...
| 写 | 不写 |
|----|------|
| 桌上 UI 拍：HandIdle → Peek / MultiSelected → ConfirmReady → **PlayLaunch** → **PlayLanded** →（hand>0）**AwaitChallenge 入口** /（hand==0）**RankSettle** | 第2节质疑四拍、押注窗、翻验动画、裁定演出 |
...
stateDiagram-v2
  ...
  PlayLanded --> AwaitChallenge: 权威成功且 hand大于0
...
| **AwaitChallenge** | `TURN`（下家） | 身前扇保持；质疑入口按 `canChallenge` | **只验入口。** 四拍 = **第2节详** | hand==0 还进来；第一手放开质疑；旧冻轨并存 |
...
| U03a | PlayLanded | `PILE_FORMED` | 权威已接受 **且** 提交后 `hand.count > 0` | **AwaitChallenge** | 质疑入口。第一手 `_disabled`（G-Q6）。其后按 `canChallenge` |
...
AUTO_PLAY 也走 U01→U02→U03a/b，不另开无飞出支路。打光的 AUTO_PLAY 同样必须扇成形后才 RankSettle。
...
| 等 ack 才飞 | **禁** |
| 分流 | 接受且 PlayLanded 后：hand>0 → AwaitChallenge；hand==0 → RankSettle（禁 AwaitChallenge） |
...
### 6.1 AwaitChallenge 入口（只入口 · 第2节详）
...
hand==0 还出现可点质疑 = **打光进了质疑等待**。

### 6.2 第一手（未打光）

本轮第一手且 hand>0：仍进 AwaitChallenge 入口。`canChallenge==false`（G-Q6）。钮 `_disabled`，文案 `lb_str_challenge_disabled_first`。  
**解冻 ≠ 第一手能质疑。**
...
| D4 | 出牌后质疑入口常冻 | 旧冻轨 | hand>0 才进 AwaitChallenge；第一手仍 disabled |
| D5 | 旧冻轨与新 AwaitChallenge 同时挂 | 旧质疑双轨禁并存 | 只留新轨 |
| D6 | 确认后直接进四拍，或打光仍进 AwaitChallenge | 入口未成形 / S16-9 | hand>0：PlayLanded→入口；hand==0：PlayLanded→RankSettle |
...
*维护人：策划文档岗 · 2026-09-13 · docs only · v0.2.0 Aron 打光名次 · 会签前零 ets · 合入 ≠ 终验*
